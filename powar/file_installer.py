...
class FileInstaller:
    ...
    def _get_files_to_update(self) -> Iterable[Tuple[str, str]]:
    ...
    def _install_file(self, source: str, dest: str, content: str) -> None:
        try:
            ...
            owner_of_dest = getpwuid(os.stat(os.path.dirname(dest)).st_uid).pw_name

        command = ("tee", dest)

        if not owner_of_dest == self._current_user:
            if not self._settings.switch_to_root:
                raise UserError(f"installing at \"{dest}\" requires to be in root mode")
            command = ("sudo", "-E", *command)

        subprocess.run(command, input=str.encode(content + '\n'), check=True, capture_output=True)
    ...
    def _run_exec(self, command: str) -> None:
        if not self._settings.dry_run:
            ...
            run_command(rendered, self._directory, return_stdout=False)
